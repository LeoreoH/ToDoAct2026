...
app.post('/actualizar-estilo', async (req, res) => {
    ...
    [estilo, req.session.usuario]  // en lugar de estiloNormalizado
);

    res.json({ ok: true });
});

app.get('/api/astronomia/contenido/:slug', async (req, res) => {
    ...
    try {
        ...
        const progreso = await client.query(`
            SELECT diagnostico_realizado, nivel_asignado
...
app.get('/api/astronomia/archivo/:slug/:nivel', async (req, res) => {
    ...
    try {
        ...
        if (!archivo.rows.length) {
            return res.status(404).json({ error: 'Archivo no registrado' });
        }
...
app.get('/api/astronomia/abrir-contenido/:slug/:nivel', async (req, res) => {
  ...
  try {
    ...
    const sql = `
      SELECT ca.archivo
      FROM contenidos c
      JOIN contenidos_archivos ca
        ON ca.contenido_id = c.id
      JOIN usuarios u
        ON u.usuario = $1
      WHERE c.slug = $2
        AND ca.nivel = $3
        AND ca.estilo = u.estilo_aprendizaje
        AND c.activo = true
    `;
    ...
    res.json({
      archivo: r.rows[0].archivo
    });

  } catch (e) {
    console.error(e);
    res.status(500).json({ error: 'Error al abrir contenido' });
  }
...
// Uso del sistema adaptativo basa en reglas para el exammen diagnostico
...
app.post('/api/diagnostico/guardar', async (req, res) => {
    ...
    const { contenido_id, respuestas } = req.body;
    const usuario = req.session.usuario;
    ...
    try {
        ...
        await client.query(`
            ...
            VALUES ($1, $2, true, $3, false, CURRENT_TIMESTAMP)
            ON CONFLICT (usuario, contenido_id)
            DO UPDATE SET
                diagnostico_realizado = true,
                nivel_asignado        = $3,
                fecha                 = CURRENT_TIMESTAMP
        `, [usuario, contenido_id, nivel_asignado]);
        ...
        await client.query(`
            ...
            FROM usuarios u
            WHERE u.usuario = $1
        `, [usuario, contenido_id, puntaje, total - correctas]);

        // 8. Respuesta con todo lo necesario para redirigir al alumno
        res.json({
            success:        true,
            correctas,
            total,
...
app.post('/api/progreso/nivel', async (req, res) => {
    ...
    const { contenido_id, nivel_completado, puntaje, aprobado } = req.body;
    const usuario = req.session.usuario;
    ...
    try {
        ...
        // 2. Obtener progreso actual para no bajar de nivel si ya tiene uno mayor
        ...
        await client.query(`
            ...
            FROM usuarios u
            WHERE u.usuario = $1
        `, [usuario, contenido_id, nivel_completado, nivel_completado, puntaje]);
...
// Visualizacion de los niveles dependiendo el resultado del examen diagnostico
