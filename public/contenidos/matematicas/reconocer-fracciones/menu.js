...
async function cargarEstadoContenido() {
    try {
        const res = await fetch(
            `/api/contenido/${AREA_CONTENIDO}/${SLUG_CONTENIDO}`,
            { credentials: 'include' }
        );
        ...
        console.error('Error al cargar estado del contenido:', e);
    }
}
...
async function abrirNivel(nivel) {
    try {
        ...
        console.error('Error al abrir nivel:', e);
        alert('Error al abrir contenido');
    }
}

function volver() {
    window.location.href = '/contenidos/matematicas/menu.html';
}
